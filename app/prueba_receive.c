...
uint8_t rx_buffer[PAYLOAD_LENGTH] = "ERROR\n";
...
int main(void)
{
    ...
    sx126x_status_t set_rx_status = sx126x_set_rx(&sx1262_context, 1000);
...
static void on_rx_done(void)
{
    ...
    sx126x_get_rx_buffer_status(&sx1262_context, &rx_buffer_status);
    sx126x_read_buffer(&sx1262_context, rx_buffer_status.buffer_start_pointer, rx_buffer, rx_buffer_status.pld_len_in_bytes);

    if (memcmp(rx_buffer, test_msg, PAYLOAD_LENGTH) == 0)
    {
    ...
    HAL_UART_Transmit(&huart2, rx_buffer, PAYLOAD_LENGTH, HAL_MAX_DELAY);

    HAL_Delay(1000);

    sx126x_set_rx(&sx1262_context, SX126X_RX_SINGLE_MODE);
}
...
static void on_rx_timeout(void)
{
    ...
    sx126x_set_rx(&sx1262_context, 1000);
}
...
static void sx1262_irq_process(void)
{
    if (sx1262_irq_fired == true)
    {
        ...
        if ((irq_reg & SX126X_IRQ_RX_DONE))
        {
            sx126x_handle_rx_done(&sx1262_context);
            on_rx_done();
        }
...
static void sx1262_radio_init(void)
{
    ...
    sx126x_set_pa_cfg(&sx1262_context, &pa_config); // ¿Quitar?
    sx126x_set_tx_params(&sx1262_context, 14, SX126X_RAMP_40_US); // range [-17, +22] for sub-G, range [-18, 13] for 2.4G ( HF_PA ) ¿Quitar?
